function [Z,F] = Assembly_SIE_par(SCOIL,freq) 
...
[Z] = assembly_ns_par(index,etod,node,elem,Z,GL_order,Index_elem,ko);
...
[Z] = assembly_ea_par(index,etod,node,elem,Z,GL_order,Index_elem,ko);
...
[Z] = assembly_va_par(index,etod,node,elem,Z,GL_order,Index_elem,ko);
...
[Z] = assembly_st_par(index,etod,node,elem,Z,GL_order,Index_elem,ko);
...
% Z = -(eta/(4*pi)) * Z; % orginally was this, but I just get the positive
Z = (eta/(4*pi)) * Z;
...
for pnum = 1:Nports
    [Vp] = excitation_coil(B_o,index, ,edge,port(pnum));
    F(:,pnum) = Vp;
end
